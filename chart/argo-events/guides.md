...
```console
helm install my-argo-events oci://dhi.io/argo-events-chart --version <version> \
...
This chart remaps JetStream EventBus sidecar images to Docker Hardened Images (`dhi/nats`,
`dhi/prometheus-nats-exporter`, and `dhi/nats-server-config-reloader`). Deprecated NATS Streaming EventBus images are
blanked because they are not in the catalog; use JetStream EventBus resources instead.

#### Step 4: Verify the installation

The controller deployment should become ready shortly after install:

```bash
$ kubectl get pods -n argo-events -l app.kubernetes.io/instance=my-argo-events
NAME                                                         READY   STATUS    RESTARTS   AGE
my-argo-events-controller-manager-xxxxx    1/1     Running   0          30s
```
