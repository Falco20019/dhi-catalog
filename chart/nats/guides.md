...
The `--set` path below (`global.image.pullSecretNames[0]`) matches the pull-secret key the chart's StatefulSet and
nats-box Deployment templates read (`.Values.global.image.pullSecretNames`).

```console
helm install my-nats oci://dhi.io/nats-chart --version <version> \
  --set "global.image.pullSecretNames[0]=helm-pull-secret"
```
...
The StatefulSet pod should show up and become ready shortly after install:

```bash
$ kubectl get pods
NAME         READY   STATUS    RESTARTS   AGE
my-nats-0    2/2     Running   0          30s
```

`2/2` includes the NATS server container and the config-reloader sidecar. A nats-box Deployment is also created for CLI
access (`kubectl exec`). Port-forward the client port and check the monitoring endpoint:

```bash
kubectl port-forward svc/my-nats 4222:4222 8222:8222
curl -s http://127.0.0.1:8222/varz | head
```

To enable the Prometheus metrics sidecar (uses `dhi/prometheus-nats-exporter`):

```console
helm upgrade my-nats oci://dhi.io/nats-chart --version <version> \
  --set "global.image.pullSecretNames[0]=helm-pull-secret" \
