...
- Kubernetes 1.25+
- Helm 3.6+
- The trust source namespace must exist before install (defaults to `cert-manager`)
...
Create the trust source namespace if it does not already exist (default is `cert-manager`), then install the chart. Make
sure you use `helm login` to log in before running `helm install`. Optionally, you can also use the `--dry-run` flag to
test the installation without actually installing anything.

By default the chart expects cert-manager to issue the webhook TLS certificate. For a quick start without cert-manager,
enable Helm-managed webhook certificates with `--set app.webhook.tls.helmCert.enabled=true` (not recommended for
production).

```console
kubectl create namespace cert-manager --dry-run=client -o yaml | kubectl apply -f -

helm install my-trust-manager oci://dhi.io/trust-manager-chart --version <version> \
...
```bash
$ kubectl get pods -n cert-manager -l app.kubernetes.io/name=trust-manager-chart
NAME                                    READY   STATUS    RESTARTS   AGE
trust-manager-chart-xxxxxxxxxx-xxxxx    1/1     Running   0          30s
```
